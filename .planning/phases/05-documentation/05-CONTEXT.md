...
After review acceptance, a documentation agent reads the actual built code and generates reference docs optimized for future lookup — not a rehash of the spec. Gate docs (constraints, glossary, state inventory) are also in scope — scaffolded by the agent with strict heading templates. Emergent docs (error taxonomy, change protocol, dependencies) are out of scope — they grow organically during development.
...
- Doc agent scope: flow narratives + module index (post-review). Gate docs (constraints, glossary, state inventory) are scaffolded as part of Phase 5 infrastructure but are human-maintained — the agent reads them as validation inputs, not generates them. Emergent docs are a separate concern.
...
- `constraints.md` → agent checks generated docs don't reference banned patterns
