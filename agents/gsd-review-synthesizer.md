...
Use **mgrep** for semantic search, **read** to load reviewer trace reports and to spot-check cited file:line references. Use **grep** to verify code patterns mentioned in findings. Use **glob** only to confirm file existence when a citation references a file path. Do not fetch external resources.
